// Pokopia 交易查詢器 - Google Apps Script 後端
// 部署方式：延伸功能 > Apps Script > 貼上此程式碼 > 部署 > 新增部署 > 網頁應用程式
// 執行身份：我（你的帳號）／存取權限：所有人
...
function doPost(e) {
  ...
  try {
    ...
    if (body.password !== ADMIN_PASSWORD) {
      ...
      if (action === 'saveItem') result = saveItem(body.item);
      else if (action === 'deleteItem') result = deleteItem(body.id);
      else if (action === 'savePoke') result = savePoke(body.poke, body.oldName);
      else if (action === 'deletePoke') result = deletePoke(body.name);
      else result = { error: 'unknown action' };
    }
  } catch (err) {
    result = { error: err.message };
...
function savePoke(poke, oldName) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === lookupName) {
      ...
      // 若名字有變，更新 items 裡的 prices key
      if (oldName && oldName !== poke.name) {
        renamePokeInItems(oldName, poke.name);
      }
      return { ok: true, action: 'updated' };
    }
  }
